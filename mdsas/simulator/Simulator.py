...
import time
import threading
from algorithms.Server_WinnForum import RegistrationRequest, InstallationParam, RcvdPowerMeasReport, MeasReport
...
parser = ArgumentParser(
    description='Testbench-to-SAS Client Script - Provide a server address and port in order to connect to the SAS.')
...
class Simulator:
    def __init__(self, numberOfUsers, percentageMU, percentMUActive, varianceOfData, percentageMobile, secureCount,
                 socketObj, sim_steps):
        """
        ...
        for x in range(numberOfUsers):
            ...
            if x < numberMU:  # malicious node
                self.createUser("", True, self.percentageMUActive, self.generateLat(), self.generateLon(), False,
                                mobile)
            else:
    ...
    def sendData(self, user, data):
        ...
        for data_point in data:
            payload.append(
                RcvdPowerMeasReport(measFrequency=str((iter * 624999) + 3550000000), measBandwidth=freqPerReport,
                                    measRcvdPower=data_point))
            # if(printToCSV):
            #     report[str((iter*625000)+3550000000)] = data_point
            iter = iter + 1

        self.socket.emit("spectrumData", json.dumps({"spectrumData": {"cbsdId": user.id, "latitude": user.latitude,
                                                                      "longitude": user.longitude,
                                                                      "spectrumData": MeasReport(payload).asdict()}}))
        # self.socket.emit("simCheckPUAlert", json.dumps({"reportId":str(self.reportId)}))
        # if(isPuData):
        #     formattedTime = str(float("{:0.3f}".format(time.time())))
        #     print("ReportID: "+str(self.reportId)+" - Adding PU at time: "+ formattedTime)
...
def printResults(numberOfUsers, percentageMU, percentageMUActive, varianceOfData, percentageMobile, secureCount,
                 sim_path):
...
def main(args):
    ...
    if (path):  # If looping path is provided, then run multiple simulations
        ...
        for entry in sim_data[""]:
            ...
            sim_one = Simulator(numberOfUsers, percentageMU, percentageMUActive, varianceOfData, percentageMobile,
                                secureCount, clientio, sim_steps)
